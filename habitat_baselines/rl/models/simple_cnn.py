...
class SEMSEGCNNNonOracle(nn.Module):
    ...
    def __init__(self, observation_space, device, output_size):
        super().__init__()
        self.device = device
        ...
        if self._n_input_semantic > 0:
            cnn_dims = np.array(
                (128, 128), # observation_space.spaces["semantic"].shape[:2],
                dtype=np.float32
            )
    ...
    def forward(self, semantic_image):
        ...
        if self._n_input_semantic > 0:
            ...
            # print("[CHECK] SEMSEGCNNNonOracle: semseg -2", semantic_observations.shape)
